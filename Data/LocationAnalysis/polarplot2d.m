function [Xi,Yi,Zi] = polarplot2d(theta, radius,varargin)
...
%p.angularrange = p.angularrange + pi/2;
...
%angl = p.angularrange;                               % angle  vector
angl = linspace(min(theta)-pi/2, max(theta)-pi/2);
...
%    end
   ...
   % Plot the polar axis
   hold on;
   line(xa,ya,'Color',p.polaraxiscolor,'LineWidth',2);
...
%       end
      ...
      % Draw the tick marks
      line(xt(1:2,:),yt(1:2,:),'Color',p.tickcolor);
...
if r > 1 || m > 1
   ...
   rho = linspace(0, max(p.radialrange));
   [xx,yy] = meshgrid(angl,rho);                        % mesh   matrices
   ...
   [xi,yi] = pol2cart(theta,rad);               % convert to Cartesian
   ...
   if length(azmgrid) > 2
      ...
      yr = yi(:,1:u:end);
%      zr = zi(:,1:u:end);
      plot(xr,yr,p.gridstyle,'Color',p.gridcolor,'LineWidth',1);
